...
The link layer needs to be able to be able to transport payloads of at least 68 bytes if it wants to be able to transport IPv4 packets.
For IPv6 this value is 1280 bytes large.

As using any form of encapsulation for tunneling adds overhead, the MTU inside of an IP tunnel (also called inner MTU or in-tunnel MTU) is always smaller than the MTU of der underlay (outer MTU / underlay MTU).
...
\todo{\href{https://en.wikipedia.org/wiki/Internet_censorship}{link}}
...
\todo{\href{https://en.wikipedia.org/wiki/Internet_censorship_circumvention}{link}}
