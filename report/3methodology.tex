...
For measuring the latency, loss, and duplication, we first calculate a hash of the IP payload of all packets before and after the router.
If we assume that all pre packets are unique then we can simply match pre and post packets based on their hash.
...
We can also measure the latency for each packet.
...
This is because with high probability, not many bits of the iperf3 counter value change from one packet to the next, resulting in only one fragment being different.
...
In order to save some space when storing the packet captures (\path{.pcap} files \cite{wiki:Pcap}), compression can be used to reduce the storage requirements.
...
To find the end of the test, we find the last packet which is larger than the same threshhold and also ignore all packets which are sent after the duration of the test is over.


\todo[inline]{convert the following to full sentences in a coherent text}
...
\todo[inline]{describe one second buckets}
\todo[inline]{Aufschreiben, warum erfordert Analysesetup, dass Pakete einzigartig sind? Beschreiben, wie man es verbessern könnte?}
\todo[inline]{describe in which direction the data is flowing}
